...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (!email || !password) {
    return next(new AppError('Por favor, informe e-mail e senha', 400));
  }
...
//TODO: verificar se o cara pertence a instituição
exports.isOwner = (Model, idField) =>
  catchAsync(async (req, _, next) => {
    if (!['admin', 'admin-instituicao'].includes(req.user.role)) next();

    const doc = await Model.findById(req.params.id).exec();

    if (req.user.id !== doc[idField].id) {
      next(new AppError('Não autorizado', 403));
      return;
    }
...
exports.forgotPasswordCode = catchAsync(async (req, res, next) => {
  // 1) Get user based on POSTed email
  const user = await User.findOne({
    email: req.body.email,
  });
  if (!user) {
    return next(new AppError('Não existe usuário associado a este email', 404));
  }

  // 2) Generate the random reset token
  const resetToken = user.createPasswordResetTokenCode();
  await user.save({
